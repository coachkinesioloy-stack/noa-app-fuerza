...
function Biomarcadores({ user }) {
  ...
  const cargar=async()=>{
    const sb=await getSB();if(!sb){setLoading(false);return;}
    const today=new Date().toISOString().split("T")[0];
    const {data}=await sb.from("biomarcadores").select("*").eq("atleta_id",user.id).eq("fecha",today).maybeSingle();
  ...
  const guardar=async()=>{
    ...
    const {data:existing,error:_existErr}=await sb.from("biomarcadores").select("id").eq("atleta_id",user.id).eq("fecha",today).maybeSingle();
    let err;
    if(existing?.id){
